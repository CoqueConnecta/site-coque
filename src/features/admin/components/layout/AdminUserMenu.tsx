...
export function AdminUserMenu({ user, isLoading, onLogout }: AdminUserMenuProps) {
  ...
  if (isLoading) {
    return (
      <div className="rounded-md border border-[var(--admin-border)] bg-[var(--admin-surface-2)] px-3 py-2.5">
        <div className="flex items-center gap-3">
          <div className="h-9 w-9 animate-pulse rounded-md bg-[var(--admin-border-sub)]" />
  ...
  return (
    <div ref={containerRef} className="relative">
      <button
        ...
        className="w-full rounded-md border border-[var(--admin-border)] bg-[var(--admin-surface-2)] px-3 py-2.5 text-left transition hover:border-[var(--admin-border-sub)] hover:bg-[var(--admin-surface-2)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--admin-focus)]"
        aria-haspopup="menu"
        aria-expanded={isOpen}
        aria-label="Menu do usuario"
      >
        <div className="flex items-center gap-3">
          <div className="h-9 w-9 shrink-0 overflow-hidden rounded-md bg-gradient-to-br from-indigo-500 to-violet-600">
            {hasPhoto ? (
              <img
                src={photoUrl}
                alt={displayName}
      ...
      {isOpen ? (
        <div
          role="menu"
          className="absolute bottom-full left-0 right-0 z-20 mb-2 rounded-md border border-[var(--admin-border-sub)] bg-[var(--admin-surface)] p-1.5 shadow-lg"
        >
          <button
            type="button"
            disabled
            className="flex w-full items-center gap-2 rounded px-3 py-2 text-left text-sm text-[var(--admin-text-4)]"
          >
          ...
          <button
            ...
            role="menuitem"
            onClick={onLogout}
            className="flex w-full items-center gap-2 rounded px-3 py-2 text-left text-sm font-medium text-[var(--admin-text-2)] transition hover:bg-red-50 hover:text-red-700"
          >
